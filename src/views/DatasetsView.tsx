...
export const DatasetsView: React.FC<DatasetsViewProps> = ({ onNavigate }) => {
  ...
  const [selectedDataset, setSelectedDataset] = useState<Dataset>(datasets[0] || polarDataService.getDatasets()[0]);
  const [downloadSuccess, setDownloadSuccess] = useState<string | null>(null);

  useEffect(() => {
    const unsub = polarDataService.subscribe(() => {
      const updated = polarDataService.getDatasets();
      setDatasets(updated);
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              >
                ...
                <span>Download Dataset ({selectedDataset.fileSize})</span>
              </button>
            </div>
            ...
            <div className="text-xs text-slate-400 mt-1 flex flex-wrap items-center gap-3">
              <span>Location: {selectedDataset.location}</span>
              <span>•</span>
              <span>Samples: {selectedDataset.samplesCount.toLocaleString()}</span>
              <span>•</span>
              <span className="font-mono text-cyan-300">DOI: {selectedDataset.doi}</span>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-[#050f1d] border border-cyan-950 relative">
            <div className="text-xs font-semibold text-slate-400 mb-2 flex items-center justify-between">
              ...
              <span className="text-cyan-400 font-mono">
                Latest: {selectedDataset.dataPoints[selectedDataset.dataPoints.length - 1].value}{' '}
                {selectedDataset.dataPoints[0].unit || ''}
              </span>
            </div>
